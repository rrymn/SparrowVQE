...
import torch
from llava_vqe.model import *
from llava_vqe.constants import DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN
from llava_vqe.model.multimodal_encoder.builder import build_vision_tower
from .multimodal_projector.builder import build_vision_projector
from llava_vqe import logger
...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", device="cuda"):
    ...
    if 'llava' in model_name.lower() or 'Sparrow' in model_name.lower():
        ...
        if 'lora' in model_name.lower() and model_base is not None:
            ...
            assert 'Sparrow' in model_name.lower(), 'The model name must contain `Sparrow`'
            lora_cfg_pretrained = SparrowConfig.from_pretrained(model_path)
            tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False, trust_remote_code=True)
            if 'Sparrow' in model_name.lower():
                model = SparrowForCausalLM.from_pretrained(model_base, config=lora_cfg_pretrained, **kwargs)
            else:
            ...
            if 'phi2' in model_name.lower() or 'Sparrow' in model_name.lower():
                logger.info(f'model_base:, {model_base}')
                config = SparrowConfig.from_pretrained(model_path, trust_remote_code=True)
                model = SparrowForCausalLM.from_pretrained(model_base, **kwargs)
            ...
            if 'phi2' in model_name.lower() or 'Sparrow' in model_name.lower():
                ...
                model = SparrowForCausalLM.from_pretrained(model_path, **kwargs)
                logger.info('Model is loaded...')
            else:
